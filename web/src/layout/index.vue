...
watchEffect(() => {
  if (isMobile.value) {
    // Mobile
    appStore.setCollapsed(true) // 移动端折叠侧边栏
    appStore.setFullScreen(false) // 移动端不使用全屏
  }

  if (isPad.value) {
    // IPad
    appStore.setCollapsed(true) // 平板折叠侧边栏
    appStore.setFullScreen(false) // 平板不使用全屏
  }

  if (isPC.value) {
    // PC
    appStore.setCollapsed(false) // PC端展开侧边栏
    appStore.setFullScreen(true) // PC端使用全屏
  }
})
</script>
